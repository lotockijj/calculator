...
public class Calculator implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try{
            while (true) {
                ...
                /*if(oneOperatorIsPushedAfterAnother()){ // when user pushed one operator after another
                    System.out.println("state: " + state + " result: " + result + " number1: " + number1);
    ...
    private boolean oneOperatorIsPushedAfterAnother(){ // when user pushed one operator after another
        if(state == State.PLUS){                       // the last operator is used after that...
            if(src == buttonMinus){
                state = State.MINUS;
                return true;
                }
